...
"""
vaayu.prelude.attrdict Tests
"""

import numpy as np
from numpy.testing import assert_allclose
from vaayu.prelude.attrdict import AttrDict

def test_attrdict_access():
    dmap = AttrDict(
        a=1, b=2,
        c=AttrDict(x=10, y=20, z=[10, 20, 2.0e-5]))
...
def test_attrdict_yaml_input():
    dmap = AttrDict.from_yaml(yaml_sample)
...
def test_attrdict_yaml_dump():
    dmap = AttrDict.from_yaml(yaml_sample)
    txt = dmap.to_yaml(default_flow_style=True)
    assert txt

def test_attrdict_json_input():
    dmap = AttrDict.from_json(json_sample)
    tmp = dmap.nalu_preprocess
    assert len(tmp.tasks) == 2
    assert tmp.init_abl_fields.fluid_parts[0] == "Unspecified-2-HEX"
